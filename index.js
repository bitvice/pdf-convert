...
function PdfConvert(pdfFilePath, options) {
	if (!options) options = {};
...
PdfConvert.prototype = {
	...
	constructConvertCommandForPage: function (pageNumber) {
		...
		var pathToConvert = path.resolve(__dirname, "lib/bin/convert.exe");

		return `${pathToConvert} ${convertOptionsString ? convertOptionsString + " " : ""} ${pdfFilePath}[${pageNumber}] ${outputImagePath}`
	},
